...
export default function OAuthCallbackScreen() {
  ...
  useEffect(() => {
    ...
    if (access_token) {
      handleGoogleToken(access_token as string, (role as string) || 'PROPRIETAIRE');
    } else {
    ...
    try {
      ...
      try {
        ...
        if (user.role !== 'PROPRIETAIRE' && user.role !== 'ADMIN') {
          throw new Error('Accès refusé. Ce compte n\'est pas un propriétaire.');
        }
        ...
        console.log('OAuth Callback: User not found or login failed, registering...', loginErr?.response?.data || loginErr?.message);
