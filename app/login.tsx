...
const LoginScreen = () => {
  ...
  const handleGoogleToken = async (accessToken: string) => {
    try {
      ...
      try {
        console.log('[Google Pro] Attempting login for existing partner:', googleEmail);
        ...
        console.log('[Google Pro] User not found, registering new partner…');
        // Compte inexistant → inscription automatique en tant que Propriétaire
        try {
          ...
          console.log('[Google Pro] Partner registered:', JSON.stringify(regRes.data));
          ...
          if (errMsg.includes('déjà utilisé') || errMsg.includes('déjà existant') || errMsg.includes('already used')) {
            ...
            Alert.alert('Erreur', `Impossible de créer le compte partenaire : ${errMsg}`);
          }
        }
      }
    } catch (error: any) {
      console.error('[Google Pro] Error:', error);
        ...
        >
          ...
          <View style={styles.header}>
            <View style={styles.logoContainer}>
              <Ionicons name="business" size={40} color="#0284c7" />
            </View>
            <Text style={styles.title}>AttouHome Pro</Text>
            <Text style={styles.subtitle}>Gérez vos biens et vos visites en toute sérénité.</Text>
          </View>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.footer}>
              <Text style={styles.footerText}>Pas de compte Pro ? </Text>
              <TouchableOpacity onPress={() => router.push('/register')}>
                <Text style={styles.registerLink}>Devenir partenaire</Text>
              </TouchableOpacity>
            </View>
          </View>
        </ScrollView>
